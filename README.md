...
> WARN! the embedding db generated is about 349GB
...
``` bash
python lib/use-embedding.py --model-name /data-ssd/chen/starencoder --embeddings-db-path /data-ssd/chen/embeddings.duckdb --function-db-path /data-ssd/chen/contracts.duckdb --limit 5 \
...
Query embeddings using model starencoder
Top 5 matches for query " function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        uint amountTokenMin,
        uint amountETHMin,
...
You can also use natural language to do the query:

``` bash
python lib/use-embedding.py --model-name /data-ssd/chen/starencoder --embeddings-db-path /data-ssd/chen/embeddings.duckdb --function-db-path /data-ssd/chen/contracts.duckdb --limit 5    'give me some  functions which doing blacklist operations, they must have function body'
Query embeddings using model starencoder
Top 5 matches for query "give me some  functions which doing blacklist operations, they must have function body":
Function source code:
